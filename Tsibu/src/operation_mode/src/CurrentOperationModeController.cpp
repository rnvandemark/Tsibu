...
bool CurrentOperationModeController::process()
{
	...
	OperationMode next_state = ((current_hunger_level == HungerLevel::FULL) || (current_hunger_level == HungerLevel::SOMEWHAT_HUNGRY)) ? OperationMode::EXPLORING : OperationMode::FINDING_FOOD;
	fsm->set_current_state(new OperationMode(next_state));
	return current_state_int != static_cast<int>(next_state);
}
